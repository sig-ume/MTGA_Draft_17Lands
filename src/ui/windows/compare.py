...
class ComparePanel(ttk.Frame):
    ...
    def refresh(self):
        ...
        self.entry_card.set_completion_list(
            [localization.get_display_card_name(v.get("name", "")) for v in card_map.values()]
        )
    ...
    def _add_card(self, event=None):
        ...
        found = next(
            (
                d
                for d in card_map.values()
                if d.get("name", "").lower() == typed
                or localization.get_display_card_name(d.get("name", "")).lower() == typed
            ),
            None,
        )
    ...
    def _update_content(self):
        ...
        for idx, card in enumerate(self.compare_list):
            ...
            for field in self.table_manager.active_fields:
                if field == "name":
                    row.append(localization.get_display_card_name(card.get("name", "")))
                elif field == "colors":
