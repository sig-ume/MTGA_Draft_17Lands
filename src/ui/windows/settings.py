...
class SettingsWindow(tkinter.Toplevel):
    ...
    def _build_ui(self):
        ...
        ttk.Label(container, text="Display Language:", foreground=Theme.TEXT_MUTED).grid(
            row=3, column=0, sticky="e", padx=5
        )
        self.vars["display_language"] = tkinter.StringVar()
        lang_om = ttk.OptionMenu(
            container,
            self.vars["display_language"],
            "",
            *constants.LANGUAGE_OPTIONS,
            style="TMenubutton",
        )
        lang_om.grid(row=3, column=1, sticky="ew", pady=2)
        ...
        size_om.grid(row=4, column=1, sticky="ew", pady=2)
    ...
    def _load_settings(self):
        ...
        self.vars["display_language"].set(s.display_language)
